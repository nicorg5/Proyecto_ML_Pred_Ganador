...
class SoccerdataValidator:
    ...
    def _check_completeness(self) -> dict[str, object]:
        ...
        with self.conn.cursor() as cur:
            ...
            if missing_xg > 0:
                self.warnings.append(
                    f"WARNING: {missing_xg} completed matches without xG data"
                )
    ...
    def _check_consistency(self) -> dict[str, object]:
        ...
        with self.conn.cursor() as cur:
            ...
            if bad_points > 0:
                self.issues.append(
                    f"CRITICAL: {bad_points} standings rows with incorrect points"
                )
    ...
    def _check_advanced_stats_coverage(self) -> dict[str, object]:
        ...
        with self.conn.cursor() as cur:
            ...
            if total_completed > 0:
                ...
                if pct < 50:
                    self.warnings.append(
                        f"WARNING: Only {pct}% of matches have advanced stats"
                    )
    ...
    def _check_quality(self) -> dict[str, object]:
        ...
        with self.conn.cursor() as cur:
            ...
            if big_scores > 0:
                self.warnings.append(
                    f"WARNING: {big_scores} matches with scores > 10 goals"
                )
    ...
    def generate_report(self) -> str:
        ...
        if sources:
            lines.append(f"  Source coverage: MatchHistory={sources.get('match_history', 0)}, "
